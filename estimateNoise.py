...
def estimateNoise(imageChannelMatrix, numSamples):
	...
	while len(pixelNoise) < numSamples:
		...
		randomRow = random.randint(1,numRows - 1)
		randomCol = random.randint(1,numCols - 1)
		
		pixelVal = imageChannelMatrix[randomRow, randomCol]
		...
		for i in range(randomRow - 1, randomCol + 2):
			for j in range(randomCol - 1, randomCol + 2):
				if i != 0 or j != 0: #we're not comparing our pixel to itself
					...
					neighborDifferences += (imageChannelMatrix[i,j] - pixelVal)**2
